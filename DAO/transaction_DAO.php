...
class transaction_DAO
{
    ...
    {
        $query = "SELECT t.transact_id, date_format(t.transact_date, '%m/%d/%Y %r %p') transact_date, t.customer_id, c.customer_name, t.discount_id FROM TRANSACT t, CUSTOMER c WHERE c.customer_id = ? AND c.customer_id = t.customer_id";
    ...
    {
        $query = "SELECT t.transact_id, date_format(t.transact_date, '%m/%d/%Y %r %p') transact_date, t.customer_id, c.customer_name, t.discount_id FROM TRANSACT t, CUSTOMER c WHERE c.customer_id = t.customer_id";
        $prepared_query = mysqli_prepare($this->_connection,$query);
        $prepared_query->execute();
        $result = $prepared_query->get_result();
        if($result->num_rows>0)
    ...
    {
        $query = "SELECT t.transact_id, date_format(t.transact_date, '%m/%d/%Y %r %p') transact_date, t.customer_id, c.customer_name, t.discount_id FROM TRANSACT t, CUSTOMER c WHERE transact_id = ? AND c.customer_id = t.customer_id";
        $prepared_query = mysqli_prepare($this->_connection,$query);
